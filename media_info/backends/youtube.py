...
try:
    ...
    yt_service = None
    print('Warning: gdata or dateutil not found, some information won\'t be retrieved')

DOMAINS = ('youtube.com', 'youtu.be')

video_id_re = re.compile(r'/(?:watch\?.*v=)?([a-zA-Z0-9_-]{11})')
def get_media_info(_, url):
    video_id = video_id_re.search(url.path + '?' + url.query)
    if not video_id:
        raise MediaInfoException(_('Unrecognized url: %s') % url.geturl())
    ...
    try:
        ...
        if not info:
            errors.append(_('Recieved empty response from %s') % info_url)
    ...
    if not r.title:
        if errors:
            error = _('Youtube said:\n"%s"') % max(errors, key=len)
        else:
            error = _('unknown reason')
        raise MediaInfoException(_('Getting video info failed, %s') % error)
